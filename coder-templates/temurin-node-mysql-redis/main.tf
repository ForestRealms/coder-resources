...
data "coder_parameter" "node_version" {
  ...
  option {
    name  = "Node 24 LTS"
    value = "harbor.cluster.internal/coder-images/node:24"
  }
...
data "coder_parameter" "install_webstorm" {
  name      = "Install JetBrains WebStrom"
  type      = "bool"
  mutable   = false
  order     = 20
  default   = true
...
module "personalize-jdk" {
  count    = data.coder_workspace.me.start_count
  source   = "registry.coder.com/coder/personalize/coder"
  version  = "1.0.32"
  agent_id = coder_agent.jdk.id
}

module "personalize-node" {
  count    = data.coder_workspace.me.start_count
  source   = "registry.coder.com/coder/personalize/coder"
  version  = "1.0.32"
  agent_id = coder_agent.node.id
}
...
resource "docker_volume" "home_volume_node" {
...
resource "docker_container" "workspace_jdk" {
  count = data.coder_workspace.me.start_count
  image = data.coder_parameter.jdk_version.value
  # Uses lower() to avoid Docker restriction on container names.
  name = "coder-${data.coder_workspace_owner.me.name}-${lower(data.coder_workspace.me.name)}-jdk"
  # Hostname makes the shell more user friendly: coder@my-workspace:~$
  hostname = data.coder_workspace.me.name
  # Use the docker gateway if the access URL is 127.0.0.1
  entrypoint = ["sh", "-c", replace(coder_agent.jdk.init_script, "/localhost|127\\.0\\.0\\.1/", "host.docker.internal")]
  env        = ["CODER_AGENT_TOKEN=${coder_agent.jdk.token}"]
  networks_advanced {
...
resource "docker_container" "workspace_node" {
  count = data.coder_workspace.me.start_count
  image = data.coder_parameter.node_version.value
  # Uses lower() to avoid Docker restriction on container names.
  name = "coder-${data.coder_workspace_owner.me.name}-${lower(data.coder_workspace.me.name)}-node"
  # Hostname makes the shell more user friendly: coder@my-workspace:~$
  hostname = data.coder_workspace.me.name
  # Use the docker gateway if the access URL is 127.0.0.1
  entrypoint = ["sh", "-c", replace(coder_agent.node.init_script, "/localhost|127\\.0\\.0\\.1/", "host.docker.internal")]
  env        = [
    "CODER_AGENT_TOKEN=${coder_agent.node.token}",
    "INSTALL_WEBSTORM=${data.coder_parameter.install_webstorm.value}"
  ]
  host {
    host = "host.docker.internal"
    ip   = "host-gateway"
  ...
  volumes {
    container_path = "/root"
    volume_name    = docker_volume.home_volume_node.name
    read_only      = false
  }
